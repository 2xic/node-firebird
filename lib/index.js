...
const
    ...
    ISOLATION_REPEATABLE_READ_60_SECONDS_TIMEOUT = [isc_tpb_version3, isc_tpb_write, isc_tpb_wait, isc_tpb_concurrency, isc_tpb_lock_timeout, 1, 60],
...
exports.ISOLATION_REPEATABLE_READ_60_SECONDS_TIMEOUT = ISOLATION_REPEATABLE_READ_60_SECONDS_TIMEOUT;
...
Database.prototype.execute = function(query, params, callback, custom) {
    ...
    self.connection.startTransaction(function(err, transaction) {
        ...
        transaction.execute(query, params, function(err, result, meta, isSelect) {
            ...
            fetch_blob_sync(result, meta, isSelect, (err, result) => {
                if (err) {
...
Database.prototype.sequentially = function(query, params, on, callback, asArray) {
    ...
    const _on = (row, i, meta, next) => {
        fetch_blob_sync([row], meta, true, function (err) {
            if (err)
                return next(err);
            on(row, i, next)
        });
            ...
            decodeResponse(xdr, cb, self, self._lowercase_keys, function (err, obj) {
                ...
                if (err) {
                    ...
                    if (self.accept.protocolMinimumType === ptype_lazy_send && self._queue.length > 0) {
                        if (self._queue && self._queue.length && self._queue[0]) {
    ...
    try {
        ...
        switch (r) {
            ...
            case op_sql_response:
                ...
                var transaction = callback.transaction;
                        ...
                        for (length = output.length; data.fcolumn < length; data.fcolumn++) {
                            ...
                            try {
                                ...
                                if (item.type === SQL_BLOB) {
                                    value = fetch_blob_async(statement, transaction, value, key);
                                }
                                data.frow[key] = value;
                            } catch (e) {
                ...
                if (r === op_cond_accept || r === op_accept_data) {
                    ...
                    if (is_authenticated === 0) {
                    	...
                    	if (AUTH_PLUGIN_SRP_LIST.indexOf(accept.pluginName) !== -1) {
							...
							// Server keys
							cnx.serverKeys = {
								salt: d.buffer.slice(2, saltLen + 2).toString('latin1'),
								public: BigInt(d.buffer.slice(keyStart, d.buffer.length).toString('latin1'), 16)
							};
        ...
        const isRangeError = err instanceof RangeError || RangeError.name === err.constructor.prototype.name;
        if (isRangeError) {
            return cb(err);
        }
        throw err;
...
function search_blob_column(row, meta) {
    var result = [];
    var asObject = Object.keys(row);
    //search if there are blob field
    for (const column in meta) {
        if (meta[column] instanceof SQLVarBlob) {
...
function readblob(blob, callback) {
    ...
    blob(function (err, name, e) {
        ...
        var _d = [];
        var _dlength = 0;
        e.on('data', function (chunck) {
            _dlength += chunck.length;
            _d.push(chunck);
        });
        e.on('end', function (chunck) {
            return callback(null, Buffer.concat(_d, _dlength));
        });
...
var fetch_blob_sync = exports.fetch_blob_sync = function(rows, meta, isSelect, callback) {
    if (!isSelect || !rows.length)
        return callback(null, rows);
    // get blob column
    var blob_col = search_blob_column(rows[0], meta);
    // no blob
    if (!blob_col.length)
        return callback(null, rows);
    var nb_blobs = rows.length * blob_col.length;
    var blob_err;

    var parallel = function(group, size) {
        if  (group*size - rows.length > size)
            return;
        var prows = rows.slice(group*size, group*size + size);
        var p_nb_blobs = prows.length * blob_col.length;
        prows.forEach(function (row) {
            blob_col.forEach(function (col) {
                readblob(row[col], function (err, data) {
                    if (err)
                        blob_err = err;
                    nb_blobs--;
                    p_nb_blobs--;
                    row[col] = data;
                    if (nb_blobs == 0) {
                        return callback(blob_err, rows);
                    }
                    if (p_nb_blobs == 0) {
                        if (blob_err)
                            return callback(blob_err, rows);
                        parallel(group + 1, size)
                    }
                })
            })
        })
    }
    parallel(0, DEFAULT_BLOB_FETCHSIZE);
}

function fetch_blob_async(statement, transaction, id, name) {
